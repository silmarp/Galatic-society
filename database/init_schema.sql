...
CREATE TABLE NACAO(
	...
	CONSTRAINT FK_NACAO_FEDERACAO FOREIGN KEY (FEDERACAO) REFERENCES FEDERACAO(NOME) ON DELETE SET NULL,
		CONSTRAINT CK_NACAO_QTD_PLANETAS CHECK (QTD_PLANETAS >= 0) -- >= 0 para poder manter o historico de nacoes extintas
);
...
CREATE TABLE LIDER(
	...
	CONSTRAINT CK_LIDER_CARGO CHECK (CARGO IN ('COMANDANTE', 'OFICIAL', 'CIENTISTA')) -- Uppercase para padronizacao
);
...
CREATE TABLE FACCAO(
	...
	CONSTRAINT CK_FACCAO_IDEOLOGIA CHECK (
		IDEOLOGIA IN ('PROGRESSISTA', 'TOTALITARIA', 'TRADICIONALISTA')
	),
	CONSTRAINT CK_FACCAO_QTD_NACOES CHECK (QTD_NACOES >= 0),
	-- >= 0 para poder manter o historico de faccoes extintas
...
CREATE TABLE COMUNIDADE(
	...
	-- >= 0 para poder manter o historico de comunidades extintas
...
CREATE TABLE ORBITA_ESTRELA(
	...
	-- >= para permitir orbitas circulares
